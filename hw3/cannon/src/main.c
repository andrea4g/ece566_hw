...
//#endif
  ...
  //#endif
    ...
    for (i = 0; i < rows*cols; i++) {
      printf("D pre invio --(%d,%d) - %f\n", my_cord[0], my_cord[1], flatD[i]);
    }
    //printf("quasi D\n");
    MPI_Sendrecv_replace(&flatD, rows*cols, MPI_FLOAT, shiftdest, 1, shiftsource, 1, mesh_comm, MPI_STATUS_IGNORE);
    for (i = 0; i < rows*cols; i++) {
      printf("D post invio --(%d,%d) - %f\n", my_cord[0], my_cord[1], flatD[i]);
    }
    //printf("dopo D\n");
    ...
    //printf("quasi B\n");
    MPI_Sendrecv_replace(&flatB, rows*cols, MPI_FLOAT, shiftdest, 1, shiftsource, 1, mesh_comm, MPI_STATUS_IGNORE);
    //printf("dopo B\n");
...
//    for (i = 0; i < sr_p; i++) {
      for (j = 0; j < sr_p; j++) {
        ...
        flatD = flattenize_matrix(D, rows, cols);
        MPI_Sendrecv_replace(&flatD, rows*cols, MPI_FLOAT, leftrank, 1, rightrank, 1, mesh_comm, MPI_STATUS_IGNORE);
        D = deflattenize_matrix(flatD, rows, cols);
        //up circ by 1
        MPI_Cart_shift(mesh_comm, 1, -1, &downrank, &uprank);
        flatB = flattenize_matrix(B, rows, cols);
        MPI_Sendrecv_replace(&flatB, rows*cols, MPI_FLOAT, uprank, 1, downrank, 1, mesh_comm, MPI_STATUS_IGNORE);
        B = deflattenize_matrix(flatB, rows, cols);
      }
//    }
//  }
